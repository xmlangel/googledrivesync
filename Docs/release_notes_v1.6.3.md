...
# Google Drive Sync v1.6.3 릴리즈 노트

본 버전에서는 동기화 중 발생하는 오류 해결과 사용자 편의성 개선에 집중했습니다.

## 주요 변경 사항

### 1. 버그 수정: EISDIR 오류 해결
지난 버전의 폴더 처리 개선에 이어, **이미 추적 중인 폴더의 수정 시간이 변경되었을 때** 발생하던 `EISDIR (Is a directory)` 오류를 완전히 해결했습니다. 이제 기존 폴더의 메타데이터 변경을 안정적으로 처리합니다.

### 2. 기능 개선: 실시간 동기화 로그 업데이트
그동안 로그 화면을 나갔다 들어와야만 최신 로그를 확인할 수 있었던 불편함을 개선했습니다. 이제 **로그 화면에서 동기화 진행 상황을 실시간으로 확인**할 수 수 있습니다.

---

### 기타 개선 사항
- 동기화 엔진 내부의 테스트 편의성 및 가독성 개선
- `SyncManager` 초기화 로그 보강
